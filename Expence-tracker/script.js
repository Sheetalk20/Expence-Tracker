...
function addExpense() {
    ...
    if (description && amount && date) {
        ...
        alert('Please enter valid expense details and date.');
    }
}
...
function formatDate(dateString) {
    ...
    return date.toLocaleDateString(); // e.g., 16/8/2025
}
...
function showMonthlyExpenses() {
    ...
    // Toggle: agar already visible hai, to hide kar do
    if (monthlyExpenseList.style.display === 'block') {
        monthlyExpenseList.style.display = 'none';
        return;
    }

    // Visible karne se pehle div ko clear kar do
    monthlyExpenseList.innerHTML = '';

    // Expenses ko month-wise sum kar lo
    ...
    for (const [monthYear, total] of Object.entries(monthlyExpenses)) {
        ...
        div.className = "bg-gray-100 border-2 border-gray-300 p-4 rounded-md shadow-sm transition-transform duration-200 hover:scale-105";
        div.innerText = `${monthYear}: ₹${total.toFixed(2)}`;
        monthlyExpenseList.appendChild(div);
    }
...
function downloadData() {
    ...
    const csvContent = "Description,Amount,date\n" + expenses.map(exp => 
        `${exp.description},${exp.amount},${exp.date}`).join("\n");
...
function downloadExcelData() {
    ...
    const ws_data = [["Description", "Amount", "date"]];
    expenses.forEach(exp => {
        ws_data.push([exp.description, exp.amount, exp.date]);
    });
